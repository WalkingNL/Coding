...
public:
    vector<int> maxSlidingWindow(vector<int>& nums, int k) {
        ...
        int max_index_in_window = 0;
        int start_each_window = 0;
        for (int i=1; i<nums.size(); ) {
            if (nums[max_index_in_window] < nums[i]) {
                max_index_in_window = i;
            }
            
            if (i-start_each_window < k-1) {
                i++;
                continue;
            }
            if (i-start_each_window == k-1) {
                vec.push_back(nums[max_index_in_window]);
                int distance = max_index_in_window - start_each_window;
                if (distance == 0) {
                    start_each_window++;
                    max_index_in_window = start_each_window;
                    i = max_index_in_window + 1;
                    continue;
                }
                int max_index_in_window_tmp = max_index_in_window;
                while (distance > 0) {
                    if (++i >= nums.size()) {
                        return vec;
                    }
                    if (nums[max_index_in_window_tmp] < nums[i]) {
                        max_index_in_window_tmp = i;
                    }
                    if (max_index_in_window_tmp != max_index_in_window) {
                        vec.push_back(nums[max_index_in_window_tmp]);
                    } else {
                        vec.push_back(nums[max_index_in_window]);
                    }
                    distance--;
                }
                start_each_window = max_index_in_window + 1;
                if (max_index_in_window != max_index_in_window_tmp) {
                    max_index_in_window = max_index_in_window_tmp;
                } else {
                    max_index_in_window = start_each_window;
                }
                i = max_index_in_window + 1;
...
int main(int argc, char* argv[]) {
    ...
    for (int x=389; x<=434; x++) {
        cout << "nums[" << x << "]=" << nums[x] << endl;
    }
    cout << endl;
    
    vector<int> vec = s.maxSlidingWindow(nums, k);

    if (vec.size() != nums.size()-k+1) {
        cout << "wrong" << endl;
        return 0;
    }
    cout << "nums size: " << nums.size() << endl;
    cout << "real size: " << nums.size()-k+1 << endl;
